...
private ["_triggerObj","_group","_groupSize","_triggerPosition","_nearestLocation","_newAct","_VestGroup","_LootGroup","_ZombieGroup","_avoidTerritory","_SpawnDelay","_RespawnDelay","_cnt","_triggerList","_triggerSize"];
...
_group = _triggerObj getvariable ["Zgroup", objNull];

_groupSize = _triggerObj getvariable ["ZgroupSize", 0];
...
_triggerSize = _triggerObj getvariable ["ZtriggerSize", 300];
...
if (Debug) then {
	diag_log format["EpochZ 1.0: Activating Trigger	|	Position : %1	|	GroupSize : %2	|	Near : %3 ",_triggerPosition,_groupSize,_nearestLocation];
};
...
SpawnOne = {
	...
	{
		_playerObj = selectRandom _triggerList;
		_playerPos = getPos _playerObj;
		//nul = [_group,_playerPos,_vestGroup,_lootGroup,_zombieGroup,_avoidTerritory] spawn SpawnZombie;
		nul = [_group,_triggerPosition,_vestGroup,_lootGroup,_zombieGroup,_avoidTerritory, _triggerSize] spawn SpawnZombie;
	};
};
...
//Reboot the trigger
diag_log format["EpochZ 1.0: Deactivating Trigger	|	Position : %1	|	GroupSize : %2	|	Near : %3 ",_triggerPosition,_groupSize,_nearestLocation];
