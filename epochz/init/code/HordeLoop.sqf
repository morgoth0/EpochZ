...
while {true} do 
{
	...
	if (Debug) then {
		diag_log format["EpochZ 1.0: Next Horde in %1 minutes.",_sleeptime/60];
	};
	sleep _sleeptime;
	...
	if (_nPlayer >= 4) then 
	{
		...
		{ 
			...
			if (Debug) then 
			{
				diag_log format["EpochZ 1.0: Spawning Horde near %1.",_playerName];
			};
		...
		{
			if (Debug) then {
				diag_log "EpochZ 1.0: No valid player found for the Horde";
			};
		};
	};
};
