...
if (SpawnWithinTriggerRadius) then {
...
GetZombieClass = {
	...
	if (Debug) then {
		diag_log format["EpochZ 1.0: Zombie Group Highest Compound Weight : %1",_MaxValue];
	};
	_result = ceil random (_maxValue);
	if (Debug) then {
		diag_log format["EpochZ 1.0: Randomly Selected Value : %1",_result];
	};
	{
		if((_x select 1) >= _result) exitwith
		{
			if (Debug) then {
				diag_log format["EpochZ 1.0: Selected Zombie Group : %1		Compound Weight : %2",(_x select 0),(_x select 1)];
			};
			_return = (selectRandom (_x select 0)) select 0;
		};
	}foreach _this;
	if (Debug) then {
		diag_log format["EpochZ 1.0: Selected Zombie Class : %1",_return];
	};
	_return;
};
...
{
	if (Debug) then {
		diag_log format["EpochZ 1.0: No suitable spawn location found for a Zombie near %1 ",_position];
	};
}
else
{	
	...
	if (Debug) then {
		diag_log format["EpochZ 1.0: Spawning 1 Zombie	|	Position : %1	|	Class : %2 ",_position,_zClass];
	};
