...
{
	{
		if ((HarassingZombieAtNightOnly && (daytime >= NightStartTime || daytime < NightEndTime)) || !(HarassingZombieAtNightOnly)) then 
		{
			if (isPlayer _x) then 
			{ 
				if (alive _x) then 
				{	
					...
					_group = _playerObj getvariable ["Zgroup", objNull];
					...
					if (_count < _groupSize) then 
					{
						...
						if (Debug) then 
						{
							diag_log format["EpochZ 1.0: Spawning 1 Zombie for %1.",_playerName];
						};
					};
				};
			};
		...
		if (Debug) then 
		{
			diag_log format["EpochZ 1.0: %1 Player in game.",_nPlayer];
		};
		...
		if (_nPlayer < 1) then // to avoid division by 0
		{
			if (Debug) then 
			{
				diag_log format["EpochZ 1.0: Waiting %1 seconds.",_frequency];
			};
			sleep _frequency;
		}
		else
		{
			...
			if (Debug) then 
			{
				diag_log format["EpochZ 1.0: Waiting %1 seconds.",_sTime];
			};
			sleep _sTime;
		};
