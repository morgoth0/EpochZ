...
if (Debug) then {
	diag_log format["EpochZ 1.0: Creating Trigger	|	Position : %1 	|	Radius : %2m	|	Near : %3 ",_triggerPosition,_triggerRadius,_nearestLocation];
};

// Store Variables in the trigger.

_trigger setvariable ["ZgroupSize", _groupSize, False];
...
_trigger setvariable ["ZtriggerSize",_triggerRadius, False];
